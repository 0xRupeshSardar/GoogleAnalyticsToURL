...
func crash(message string, err error) {
	...
	if err != nil {
		...
		os.Exit(1) // Exit with a non-zero status if no error object is provided
	}
}
...
func banner() {
	...
	data += "\033[32m> \033[0msubdomains by looking at Google Analytics IDs ./GoogleAnalyticsToURL-h\n"
	data += "\033[32m> \033[0mGO Version v1\n"
...
func getURLResponseWithRetry(url string, retries int) string {
	var response string
	for i := 0; i < retries; i++ {
...
func getDomainsFromBuiltWith(id string) ([]string, error) {
	pattern := regexp.MustCompile(`/relationships/[a-z0-9\-\_\.]+\.[a-z]+`)
	url := "https://builtwith.com/relationships/tag/" + id
	response := getURLResponseWithRetry(url, 3) // Retry up to 3 times
	var allDomains []string
	if response != "" {
		allDomains = cleanRelationShips(pattern.FindAllStringSubmatch(response, -1))
	} else {
		return nil, fmt.Errorf("Failed to fetch domains from BuiltWith")
	}
...
func getDomains(id string) []string {
	var allDomains []string
	domainsFromBuiltWith, err := getDomainsFromBuiltWith(id)
	if err == nil {
		allDomains = append(allDomains, domainsFromBuiltWith...)
	} else {
		warning(err.Error())
	}
	...
	if !chainMode {
		...
		for _, domain := range allDomains {
			if domain == "error getting results" {
				warning("Server-side error on builtwith.com: error getting results")
				continue
			}
			fmt.Println(domain)
		}
...
func main() {
	...
	const usage = `Usage: ./GoogleAnalyticsToURL-u URL [--chain-mode]
  -u, --url string
		...
		if (stat.Mode() & os.ModeCharDevice) == 0 {
			...
			for scanner.Scan() {
				if err := scanner.Err(); err != nil {
					crash("ERROR reading from stdin", err)
				}
				start(scanner.Text(), chainMode)
			}
		} else {
			crash("URL to extract Google Analytics ID is required!", nil)
		}
	}
}
